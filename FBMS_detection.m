function FBMS_detection(fast_rcnn_net, opts, proposal_detection_model, rpn_net)
%% script for detecting objects from FBMS dataset by Faster RCNN
% set data path
dataDir = '/BS/joint-multicut/work/FBMS59/Testset/';
Type = '*.ppm';
outMat = '/BS/joint-multicut-2/work/FBMS-fasterRCNN/Testset03/';
folder = dir(dataDir);
folder(1:2)=[];
...
for img_index=1:size(folder,1)
    ...
    for imgnum=1:LengthFiles
        ...
        for i = 1:length(boxes_cell)
            ...
            Boxes2save = [Boxes2save;boxes_cell{i}(boxes_cell{i}(:, 5) >= 0.3, :)];
